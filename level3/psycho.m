function SMR = psycho(frameT, frameType, frameTprev1, frameTprev2)
    ...
    if(frameType=='ESH')
        % Initialize ESH
        energy = zeros(42,1);
        predictability = zeros(128,1);
        predictability_2 = energy;
        ecb = energy;cb = energy;ct = energy;
        en = energy; tb = energy;NMT = energy;
        TMN = energy;SNR = energy;bc = energy;
        nb = energy;qthr_hat = energy;
        npart = energy;SMR = zeros(42,8);
        ...
        for j=1:8
            ...
            % Calculate FFT
            % Calculate norm and phase of every frequency(from 1 to 128, the next are symmetrical) 
        ...
        % Initialize non ESH
        energy = zeros(69,1);
        predictability = zeros(1024,1);
        predictability_2 = energy;
        ecb = energy;cb = energy;ct = energy;
        en = energy; tb = energy;NMT = energy;
        TMN = energy;SNR = energy;bc = energy;
        nb = energy;qthr_hat = energy;
        npart = energy;SMR = energy;
        ...
        % Calculate norm and phase of every frequency(from 1 to 1024, the next are symmetrical) 
        fourier_trans=fft(Sw_T);
        r_T= abs(fourier_trans(1:N/2));
        phase_T= angle(fourier_trans(1:N/2));
        ...
        for n=1:69
            ecb(n)= energy'*long(:, n);
            ct(n)= predictability_2'*long(:, n);

            % Normalize predictability and energy 
            cb(n) = ct(n)/ecb(n);
            en(n) = ecb(n)/ sum(long(:, n) );
